    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            <div className="container mx-auto px-4 md:px-10 py-16 md:py-20">
                ...
                {/* Logos Grid */}
                <div className="grid grid-cols-2 md:grid-cols-6 border-l border-t border-gray-100">
                    {clients.map((logo, index) => (
                        <div
                            key={index}
                            className="flex items-center justify-center p-4 h-56 border-r border-b border-gray-100 transition-shadow duration-300 bg-white"
                        >
                            <img
                                src={logo}
                                alt={`Client ${index + 1}`}
                                className="max-w-[280px] max-h-[140px] object-contain"
                            />
                        </div>
                    ))}
